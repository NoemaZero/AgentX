...
from claude_code.pydantic_models import MutableModel
...
def load_agent_memory(
    ...
    - Local: .claude/agent-memory-local/<type>/
    """
    import os
    ...
    scopes = [
        ("user", os.path.join(home, ".claude", "agent-memory", agent_type)),
        ("project", os.path.join(cwd, ".claude", "agent-memory", agent_type) if cwd else ""),
        ("local", os.path.join(cwd, ".claude", "agent-memory-local", agent_type) if cwd else ""),
    ]
