...
Registers all 6 built-in agents:
  ...
  5. help-guide — environment help guide (non-SDK only)
...
from AgentX.constants.identity import (
    get_agent_intro,
    get_app_help_name,
    get_typed_agent_intro,
)
from AgentX.data_types import AgentModel
...
def _is_sdk_entry() -> bool:
    ...
    return os.environ.get("AGENTX_SDK_ENTRY", "").lower() == "true"
...
def _explore_system_prompt(**_kwargs: Any) -> str:
    return f"""\
{get_typed_agent_intro("an Explore")}
...
def _verification_system_prompt(**_kwargs: Any) -> str:
    return f"""\
{get_typed_agent_intro("a Verification")}
...
def _help_guide_system_prompt(**_kwargs: Any) -> str:
    _app = get_app_help_name()
    return f"""\
{get_typed_agent_intro(f"a {_app} Guide")}. You help users understand how to use {_app}, \
...
from AgentX.constants.identity import get_help_agent_type

HELP_GUIDE_AGENT = BuiltInAgentDefinition(
    agent_type=get_help_agent_type(),
    when_to_use=(
        f"Helps users understand {get_app_help_name()} features, configuration, and capabilities. "
        f"Use when the user asks 'how do I...' questions about {get_app_help_name()} itself."
    ),
...
HELP_GUIDE_AGENT._get_system_prompt = _help_guide_system_prompt
...
def _statusline_setup_system_prompt(**_kwargs: Any) -> str:
    _app = get_app_help_name()
    return f"""\
{get_typed_agent_intro("a Statusline Setup")}. You help users configure \
...
STATUSLINE_SETUP_AGENT = BuiltInAgentDefinition(
    ...
    when_to_use=(
        ...
        f"user wants to set up or modify their {get_app_help_name()} status line display."
    ),
    tools=["Read", "Edit"],
...
def get_built_in_agents() -> list[BaseAgentDefinition]:
    ...
    Conditionally includes: Explore, Plan, help-guide, verification.
    """
    # Check if built-in agents are disabled (SDK mode)
    if os.environ.get("AGENTX_DISABLE_BUILTIN_AGENTS", "").lower() == "true":
        return []
    ...
    if not _is_sdk_entry():
        agents.append(HELP_GUIDE_AGENT)
