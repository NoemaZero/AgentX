...
from AgentX.commands.registry import CommandRegistry
...
from AgentX.ui.renderer import render_cost, render_error
...
async def run_repl(config: Config) -> None:
    ...
    title_text = f"🤖 {get_app_display_name()} v{__version__}"
    ...
    panel = Panel(
        banner_text,
        title=f"[bold green]🤖 {get_app_display_name()} v{__version__}[/bold green]",
        border_style="green",
        padding=(0, 1),
        title_align="left",
        width=panel_width
