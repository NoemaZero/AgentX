...
from AgentX.constants.cyber_risk import CYBER_RISK_INSTRUCTION
...
from AgentX.constants.tool_names import (
    AGENT_TOOL_NAME,
...
APP_DOCS_MAP_URL = get_app_docs_url()
...
_scratchpad_dir = "~/.agentx/scratchpad"
...
def get_simple_doing_tasks_section() -> str:
    ...
    return _format_section(
        ...
        [
            ...
            "the following:",
            [
                f"/help: Get help with using {get_app_help_name()}",
...
Carefully consider the reversibility and blast radius of actions. Generally you can freely take local, reversible actions like editing files or running tests. But for actions that are hard to reverse, affect shared systems beyond your local environment, or could otherwise be risky or destructive, check with the user before proceeding. The cost of pausing to confirm is low, while the cost of an unwanted action (lost work, unintended messages sent, deleted branches) can be very high. For actions like these, consider the context, the action, and user instructions, and by default transparently communicate the action and ask for confirmation before proceeding. This default can be changed by user instructions - if explicitly asked to operate more autonomously, then you may proceed without confirmation, but still attend to the risks and consequences when taking actions. A user approving an action (like a git push) once does NOT mean that they approve it in all contexts, so unless actions are authorized in advance in durable instructions like AgentX.md files, always confirm first. Authorization stands for the scope specified, not beyond. Match the scope of your actions to what was actually requested.
...
def _get_skill_count(cwd: str) -> int:
    ...
    """
    from AgentX.skills import get_all_skills
