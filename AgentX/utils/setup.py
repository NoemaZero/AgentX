...
- Environment checks
- Claude configuration (.agentx/ directory init)
"""
...
from AgentX.utils.git import is_git_repo, run_git_command
...
def ensure_claude_dir(cwd: str) -> Path:
    """Ensure ~/.agentx/ directory exists with proper structure.

    Creates:
    - ~/.agentx/
    - ~/.agentx/projects/
    - ~/.agentx/settings.json (if missing)
    """
...
def ensure_project_claude_dir(cwd: str) -> Path | None:
    """Ensure .agentx/ directory in the project root.

    Returns the .agentx/ path, or None if not in a project.
    """
    project_claude = Path(cwd) / ".agentx"
    try:
